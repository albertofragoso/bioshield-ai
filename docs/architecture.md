...
- **Cifrado:** Los datos de `biomarkers.encrypted_data` se cifran con **AES-256** a nivel de aplicación antes de almacenarse.
...
- **Migración MVP → Producción:** El schema es compatible con SQLite (desarrollo) y PostgreSQL (producción). Los tipos `UUID` y `JSONB` se adaptan a `TEXT` y `JSON` respectivamente en SQLite.
