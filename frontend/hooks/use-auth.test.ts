...
describe('useRegister', () => {
  it('calls register then login with same credentials', async () => {
    ...
    registerMock.mockImplementation(async (body) => { callOrder.push('register'); return {} as never })
    loginMock.mockImplementation(async (body) => { callOrder.push('login'); return {} as never })
...
describe('useLogout', () => {
  ...
  it('calls cancelQueries before removeQueries on success', async () => {
    const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false }, mutations: { retry: false } } })
    const callOrder: string[] = []
    const cancelSpy = vi.spyOn(queryClient, 'cancelQueries').mockImplementation(async () => { callOrder.push('cancel') })
    const removeSpy = vi.spyOn(queryClient, 'removeQueries').mockImplementation(() => { callOrder.push('remove') })
