...
describe('useScanResult', () => {
  it('fetches scan by id with staleTime Infinity', async () => {
    ...
    const { result } = renderHook(() => useScanResult('abc'), { wrapper: makeWrapper(queryClient) })
  ...
  it('accepts enabled override', () => {
    ...
    const { result } = renderHook(
      () => useScanResult('abc', { enabled: false }),
...
describe('useAlternatives', () => {
  it('is disabled when barcode is undefined', () => {
    const queryClient = makeClient()
    const { result } = renderHook(() => useAlternatives(undefined), { wrapper: makeWrapper(queryClient) })
    expect(result.current.fetchStatus).toBe('idle')
  })
  ...
  it('fetches when barcode is provided', async () => {
    ...
    const { result } = renderHook(() => useAlternatives('7501030'), { wrapper: makeWrapper(queryClient) })
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
...
describe('useLinkPhotoToBarcode', () => {
  it('calls linkPhotoToBarcode with pseudoBarcode and barcode', async () => {
    const mock = vi.mocked(scanApi.linkPhotoToBarcode).mockResolvedValue({ product_barcode: 'real123' } as never)
    const queryClient = makeClient()

    const { result } = renderHook(() => useLinkPhotoToBarcode(), { wrapper: makeWrapper(queryClient) })
...
describe('useCreateShareLink', () => {
  it('calls createShareLink with scanDbId', async () => {
    const mock = vi.mocked(scanApi.createShareLink).mockResolvedValue({ share_url: 'http://x', expires_at: '' })
    const queryClient = makeClient()

    const { result } = renderHook(() => useCreateShareLink(), { wrapper: makeWrapper(queryClient) })
...
describe('useRevokeShareLink', () => {
  it('calls revokeShareLink with scanDbId', async () => {
    ...
    const { result } = renderHook(() => useRevokeShareLink(), { wrapper: makeWrapper(queryClient) })
    act(() => result.current.mutate('scan-id-1'))
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
...
describe('useContributeToOff', () => {
  it('calls contributeToOff with request body', async () => {
    ...
    const { result } = renderHook(() => useContributeToOff(), { wrapper: makeWrapper(queryClient) })
...
describe('useSharedScan', () => {
  it('fetches shared scan by token', async () => {
    ...
    const { result } = renderHook(() => useSharedScan('tok123'), { wrapper: makeWrapper(queryClient) })
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
