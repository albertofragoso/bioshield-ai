...
describe('useBiomarkerStatus', () => {
  it('uses correct queryKey and staleTime', async () => {
    ...
    const { result } = renderHook(() => useBiomarkerStatus(), { wrapper: makeWrapper(queryClient) })
...
describe('useExtractBiomarkers', () => {
  it('calls extractBiomarkers API', async () => {
    const extractMock = vi.mocked(biosyncApi.extractBiomarkers).mockResolvedValue({ biomarkers: [] } as never)
    const queryClient = makeClient()

    const { result } = renderHook(() => useExtractBiomarkers(), { wrapper: makeWrapper(queryClient) })
...
describe('useUploadBiomarkers', () => {
  it('calls uploadBiomarkers API and invalidates biosync-status', async () => {
    const uploadMock = vi.mocked(biosyncApi.uploadBiomarkers).mockResolvedValue({ has_data: true } as never)
    const queryClient = makeClient()
    const invalidateSpy = vi.spyOn(queryClient, 'invalidateQueries')

    const { result } = renderHook(() => useUploadBiomarkers(), { wrapper: makeWrapper(queryClient) })
    act(() => result.current.mutate({ biomarkers: [], lab_name: null, test_date: null }))
    await waitFor(() => expect(result.current.isSuccess).toBe(true))

    expect(uploadMock.mock.calls[0][0]).toEqual({ biomarkers: [], lab_name: null, test_date: null })
...
describe('useDeleteBiomarkers', () => {
  ...
  it('invalidates biosync-status on success', async () => {
    ...
    const { result } = renderHook(() => useDeleteBiomarkers(), { wrapper: makeWrapper(queryClient) })
